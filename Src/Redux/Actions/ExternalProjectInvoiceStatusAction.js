...
export function ExternalInvoiceStatus() {
    return async dispatch => {
      dispatch(
        ExternalinvoicestatusDispatch({isLoading: true},  EXTERNALINVOICESTATUS_PROGRESS),
      );
      try {
        const res = await request({url:"/invoice-master/invoice-details?type=forExternalInvoiceStatus", method: 'GET'});
  ...
  export function ExternalInvoiceStausSearch(name) {
    ...
    return async dispatch => {
      dispatch(
        ExternalinvoicestatusDispatch({isLoading: true},   EXTERNALINVOICESTATUSSEARCH_PROGRESS),
      );
      try {
        const res = await request({url:`/invoice-master/invoice-details?type=forExternalInvoiceStatus&search=${clientname}`, method: 'GET'});
        // console.log('invoice-details Response', res);
        dispatch(
            ExternalinvoicestatusDispatch(
            res.data,
...
let datas={
  ...
  return async dispatch => {
      dispatch(ExternalinvoicestatusDispatch({}, EXTERNALINVOICESTATUSMAILEXPORT_PROGRESS))
      ...
      try {
          ...
          if (data.message) {
              dispatch(ExternalinvoicestatusDispatch(data,  EXTERNALINVOICESTATUSMAILEXPORT_SUCCESS));
          }
          Toast.show("Internal Invoice Details Sent Successfully")
      } catch (err) {
          console.log('updateVendor error', err.data);
          dispatch(ExternalinvoicestatusDispatch(err, EXTERNALINVOICESTATUSMAILEXPORT_FAIL,));
